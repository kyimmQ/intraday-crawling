...
func main() {
	...
	startSignalR = func() {
		...
		if err != nil {
			...
			// Signal reconnect if login fails
			select {
		...
		streamClient.OnError = func(err error) {
			log.Printf("SignalR Error: %v. Reconnecting...", err)
			streamClient.Close()
			select {
			case reconnectChan <- struct{}{}:
			default:
		...
		go func() {
			if err := streamClient.StartWithLoop(); err != nil {
				log.Printf("Error starting SignalR loop: %v", err)
				streamClient.Close()
				select {
				case reconnectChan <- struct{}{}:
				default:
	...
	// Wait for context cancellation or reconnect signal
	for {
		select {
		case <-ctx.Done():
