...
def combine_files():
    # Find all symbols in morning and afternoon directories
    symbols = set()
    ...
    if not symbols:
        print("No symbols found to combine.")
        return
    ...
    for symbol in symbols:
        ...
        morning_symbol_dir = os.path.join(MORNING_DIR, symbol)
        if os.path.exists(morning_symbol_dir):
            ...
            if latest_morning:
                try:
                    ...
                    dfs.append(df)
                    print(f"Loaded {latest_morning} for {symbol} ({len(df)} rows)")
                except Exception as e:
                    print(f"Error reading {latest_morning}: {e}")

        # Check afternoon directory
        afternoon_symbol_dir = os.path.join(AFTERNOON_DIR, symbol)
        if os.path.exists(afternoon_symbol_dir):
            ...
            if latest_afternoon:
                try:
                    ...
                    dfs.append(df)
                    print(f"Loaded {latest_afternoon} for {symbol} ({len(df)} rows)")
                except Exception as e:
                    print(f"Error reading {latest_afternoon}: {e}")

        if not dfs:
            print(f"No data to combine for {symbol}")
            continue
        ...
        if initial_len != deduped_len:
            print(f"Removed {initial_len - deduped_len} duplicate rows for {symbol}")
        ...
        combined_symbol_dir = os.path.join(COMBINED_DIR, symbol)
        ...
        print(f"Saved combined data for {symbol} to {out_file} ({len(combined_df)} rows)\n")
